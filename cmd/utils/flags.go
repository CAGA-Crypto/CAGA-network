...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For testnets: use --ankara instead)",
	...
	//todo caga
	AnkaraFlag = &cli.BoolFlag{
		Name:     "ankara",
		Usage:    "Ankara network: pre-configured proof-of-authority test network",
		Category: flags.EthCategory,
	}
	// Dev mode
	...
	StateSchemeFlag = &cli.StringFlag{
		Name:     "state.scheme",
		Usage:    "Scheme to use for storing CAGA state ('hash' or 'path')",
		Category: flags.StateCategory,
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(AnkaraFlag.Name) {
			return filepath.Join(path, "ankara")
		}
		return path
	}
...
// 1. --bootnodes flag
// 2. Config file
// 3. Network preset flags (e.g. --ankara)
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(AnkaraFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "ankara")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, AnkaraFlag)
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Ethereum) {
	...
	if err != nil {
		Fatalf("Failed to register the CAGA service: %v", err)
	}
...
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the CAGA Stats service: %v", err)
	}
}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(AnkaraFlag.Name):
		genesis = core.DefaultAnkaraGenesisBlock()
	case ctx.Bool(DeveloperFlag.Name):
