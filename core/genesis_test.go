...
func TestInvalidCliqueConfig(t *testing.T) {
	block := DefaultAnkaraGenesisBlock()
...
func testSetupGenesis(t *testing.T, scheme string) {
	...
	tests := []struct {
		...
		{
			name: "custom block in DB, genesis == ankara",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				tdb := trie.NewDatabase(db, newDbConfig(scheme))
				customg.Commit(db, tdb)
				return SetupGenesisBlock(db, tdb, DefaultAnkaraGenesisBlock())
			},
			wantErr:    &GenesisMismatchError{Stored: customghash, New: params.AnkaraGenesisHash},
			wantHash:   params.AnkaraGenesisHash,
			wantConfig: params.AnkaraChainConfig,
		},
		{
...
func TestGenesisHashes(t *testing.T) {
	for i, c := range []struct {
		...
		{DefaultAnkaraGenesisBlock(), params.AnkaraGenesisHash},
	} {
		// Test via MustCommit
		db := rawdb.NewMemoryDatabase()
